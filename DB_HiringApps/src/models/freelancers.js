...
module.exports = {
  ...
  listFreelancersByIDModel: (searchKey, searchValue, jobDesc, statusJob, cityAddress, sort, typeSort, limit, offset, idAccount) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      db.query(`SELECT A.id_account, A.name, F.jobDesc, F.statusJob, F.cityAddress, F.description, F.image, GROUP_CONCAT(S.skill_name) AS skill , A.numberPhone, A.email FROM account AS A INNER JOIN freelancers AS F ON A.id_account = F.id_account LEFT JOIN skill_freelancers AS SF ON F.id_account = SF.id_account LEFT JOIN skill AS S ON SF.id_skill = S.id_skill WHERE A.id_account = ${idAccount} AND ${searchKey} LIKE '%${searchValue}%' AND ${filter} GROUP BY SF.id_account ${order} LIMIT ${limit} OFFSET ${offset} `, (error, result) => {
        if (!error) {
          resolve(result)
        } else {
          reject(new Error(error))
